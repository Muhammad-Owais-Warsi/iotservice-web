...
// const vendorPrisma = require("../utils/prismaClient");
...
router.post(
    "/register-master",
    identifer(["CUERON_ADMIN"]),
    async (req, res) => {
        try {
            const { email, password, firstName, lastName, companyId } =
                req.body;
        ...
        try {
            ...
            if (req.user.role === "MASTER") {
                if (
                    ...
                    return res.status(403).json({
                        error: "You only have permission to suspend Employees in your company",
                    });
                }
            }
...
router.get("/verify", identifer(), async (req, res) => {
    try {
        ...
        if (!user || user.status === "suspended") {
            return res.json({
                success: false,
                message: "Token invalid or user suspended",
            });
        }
